...
function* addUniversityReq({ payload:{ university, isErasmus}}) {
  console.log(`add uni ${university}`);

  try {
      if (isErasmus === 'Erasmus') {
        const { data } = yield call(addErasmusUniversity, university);
      } else {
        const { data } = yield call(addExchangeUniversity, university);
      }
...
function* addLangReq({ payload:{ languageRequirement, isErasmus, id}}) {

  try {
      if (isErasmus === 'Erasmus') {
        const { data } = yield call(addErasmusLangRequirement, languageRequirement, id);
      } else {
        const { data } = yield call(addExchangeLangRequirement, languageRequirement, id);
      }
...
const universitySaga = [
  ...
  takeEvery(ADD_LANG_REQUIREMENT_REQUEST, addLangReq),
